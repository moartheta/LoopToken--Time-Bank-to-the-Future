// @TODO: Update this address to match your deployed Loop Token contract!
const contractAddress = "0x7a377fAd8c7dB341e662c93A79d0B0319DD3DaE8";

function getMatch(category) {
  //startList = [] - get from somewhere - DB?
  //outList = []

  //for each address in list
    //if startList[category] matches category
      //outList.append(startList[address])
  //return outList
}
...
const dApp = {
  ...
  collectVars: async function() {
    // get land tokens
    this.tokens = [];
    this.totalSupply = await this.LoopContract.methods.totalSupply().call();
    ...
    for (let i = 1; i <= this.totalSupply; i++) {
      try {
        const token_uri = await this.LoopContract.methods.tokenURI(i).call();
        console.log('token uri', token_uri)
        const token_json = await fetchMetadata(token_uri);
        console.log('token json', token_json)
        this.tokens.push({
          tokenId: i,
          highestBid: Number(await this.LoopContract.methods.highestBid(i).call()),
          auctionEnded: Boolean(await this.LoopContract.methods.auctionEnded(i).call()),
          pendingReturn: Number(await this.LoopContract.methods.pendingReturn(i, this.accounts[0]).call()),
          auction: new window.web3.eth.Contract(
            this.auctionJson,
            await this.LoopContract.methods.auctions(i).call(),
            { defaultAccount: this.accounts[0] }
          ),
          owner: await this.LoopContract.methods.ownerOf(i).call(),
          ...token_json
        });
      } catch (e) {
  ...
  requestWork: async function() {
    const name = $("#dapp-sow").val();
    ...
    try {
      M.toast({ html: "Uploading file to IPFS via Pinata..." });
  ...
  main: async function() {
    ...
    this.LoopJson = await (await fetch("./MartianMarket.json")).json();
    this.auctionJson = await (await fetch("./MartianAuction.json")).json();

    this.LoopContract = new window.web3.eth.Contract(
    this.LoopContract = new window.web3.eth.Contract(
      this.LoopJson,
      this.contractAddress,
      { defaultAccount: this.accounts[0] }
    );
    console.log("Contract object", this.LoopContract);

    this.isAdmin = this.accounts[0] == await this.LoopContract.methods.owner().call();
